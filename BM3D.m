...
y_est = double(y_est);
 ...
 save([[image_name],'50_bm3d','.mat'],'y_est');
psnr_guide=psnr(y,y_est)
%ssim_guide_sys=ssim((y),(y_est))
%ssim1=cal_ssim(im2uint8(y),im2uint8(y_est),0,0)
ssim_guide_sys=ssim(im2uint8(y),im2uint8(y_est))
vif_bm3d=vifvec(double(im2uint8(y_est)),double(im2uint8(y)))
close all
%psnr_guide    =   csnr(double(im2uint8(y)),double(im2uint8(y_est)),0,0)
%ssim_guide    =   cal_ssim( double(im2uint8(y)), double(im2uint8(y_est)), 0, 0 )
%figure;imshow(y_est,[]);
%%print('-depsc2','barbara_bm3d_30.eps');
%imwrite(y_est,'barbara_bm3d_30.png','png');
